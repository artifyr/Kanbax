    ...
    return (
        <div className="space-y-12 animate-soft-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                {[
                    { label: 'Overall Progress', value: '78.4%', trend: '+4.2%', icon: TrendingUp, color: 'primary' },
                    { label: 'SLA Compliance', value: '99.2%', trend: '+0.1%', icon: Gauge, color: 'lime' },
                    { label: 'Mean Resource Load', value: '64%', trend: '-2.5%', icon: BarChart3, color: 'mustard' },
                    { label: 'Quality Assurance', value: '94.8%', trend: '+1.8%', icon: TrendingUp, color: 'primary' },
                ].map((kpi, i) => (
                    <div key={i} className="bg-paper-white p-8 rounded-[2rem] shadow-soft border border-midnight/5 group hover:border-primary/20 transition-all">
                        <div className={`w-12 h-12 bg-${kpi.color}/10 text-${kpi.color} rounded-2xl flex items-center justify-center mb-6 ring-8 ring-${kpi.color}/5`}>
                            <kpi.icon className="w-6 h-6" />
                        </div>
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{kpi.label}</p>
                        <div className="flex items-end gap-3">
                            <h3 className="text-3xl font-black text-midnight tracking-tight tabular-nums">{kpi.value}</h3>
