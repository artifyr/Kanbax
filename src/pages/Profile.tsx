...
import { Camera, ChevronRight, Shield, Bell, LayoutPanelLeft, User } from 'lucide-react';
import { useTaskStore } from '../hooks/useTaskStore';

export const Profile: React.FC = () => {
    const { currentUser, updateCurrentUser } = useTaskStore();
                    ...
                    <div className="flex items-center gap-10 mb-14">
                        ...
                        <div className="mt-14 pt-10 border-t border-midnight/[0.03]">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                                <button className="p-5 rounded-3xl bg-paper-white border-[3px] border-primary/40 ring-8 ring-primary/5 flex flex-col items-center gap-4 transition-all hover:scale-[1.02]">
                                    <div className="w-full aspect-video bg-canvas-white rounded-2xl border border-midnight/5 shadow-inner" />
                                    <span className="text-[11px] font-black uppercase tracking-[0.15em] text-primary">Light Canvas</span>
                                </button>
                                <button className="p-5 rounded-3xl bg-paper-white/30 border border-midnight/[0.05] flex flex-col items-center gap-4 hover:bg-paper-white hover:border-midnight/10 group transition-all duration-300">
                                    <div className="w-full aspect-video bg-midnight/90 rounded-2xl border border-midnight/10 shadow-xl opacity-80 group-hover:opacity-100 transition-opacity" />
                                    <span className="text-[11px] font-extrabold uppercase tracking-[0.15em] text-slate-400 group-hover:text-midnight">Dark Ink</span>
                                </button>
